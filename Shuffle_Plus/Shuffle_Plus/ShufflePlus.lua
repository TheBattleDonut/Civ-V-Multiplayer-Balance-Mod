...
-- DateCreated: 3/8/2020 11:54:33 AM
--------------------------------------------------------------
...
function GetMapScriptInfo()
	...
	return {
		Name = "Shuffle Plus",
		Description = "Shuffle + Resources option",
		IsAdvancedMap = 0,
		IconIndex = 1,
        SortIndex = 1,
...
function PangaeaFractalWorld:GeneratePlotTypes(args)
	...
	local sea_level = 1 + Map.Rand(3, "Random Sea Level - Lua");
...
function GeneratePlotTypes()
	print("Generating Plot Types (Lua Shuffle) ...");
	...
	elseif userInputLandmass == 4 then -- Islands
		local sea = 1 + Map.Rand(3, "Random Sea Level - Lua");
		local age =  1 + Map.Rand(3, "Random World Age - Lua");
...
function GenerateTerrain()
	print("Generating Terrain (Lua Shuffle) ...");

	-- Get Temperature setting input by user.
	local temp = 1 + Map.Rand(3, "Random Temperature - Lua");
...
function AddFeatures()
	print("Adding Features (Lua Shuffle) ...");
...
function StartPlotSystem()
	...
	print("Choosing start locations for civilizations - Shuffle Lua.");
	local args = {};
